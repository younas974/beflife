...
function treeComission($id, $amount, $details, $bv)
        ...
        if (isUserExists($id)) {
            ...
            if ($posUser->plan_id != 0) {
                ...
                $trx =  $posUser->transactions()->create([
                    ...
                    'charge' => 0,
                    'trx_type' => '+',
                    'details' => 'Paid '. round($amount,2) . ' for ' . $bv .' BV',
                 ...
                 notify($posUser, 'binary_commission', [
                    ...
                    'post_balance' => getAmount($posUser->balance),
                        ...
                        // if right is less then the left after
                        
                        if(round($posUser->userExtra->bv_right)>round(($posUser->userExtra->bv_left)-$bv)){
    
                            foreach ($plans as $plan) {
                            
                                if((round($posUser->userExtra->bv_right+$plan->bv))== (round($posUser->userExtra->bv_left)))
                               {
                                      $posUser->balance+= $plan->tree_com;
                                      $posUser->total_binary_com += $plan->tree_com;
                                      $posUser->save();
                                      ...
                                      $trx =  $posUser->transactions()->create([
                                          ...
                                          'charge' => 0,
                                          'trx_type' => '+',
                                          'details' => 'Paid' .$plan->tree_com. 'for' .$plan->bv. '',
                                          'remark' => 'binary_commission',
                                          'trx' => getTrx(),
                                          'post_balance' => getAmount($posUser->balance),
                                       ]);
                                       ...
                                       notify($posUser, 'binary_commission', [
                                          'trx' => $trx->trx,
                                          'amount' => getAmount($plan->tree_com),
                                          'currency' => $gnl->cur_text,
                                          'username' => $fromUser->username,
                                          'post_balance' => getAmount($posUser->balance),
                                       ]);
                    ...
                    if($posUser->userExtra->bv_left >= $posUser->userExtra->bv_right){
                        ...
                        if(round($posUser->userExtra->bv_left)>round($posUser->userExtra->bv_right-$bv)){
    
                            foreach ($plans as $plan) {
                            
                                if((round($posUser->userExtra->bv_left+$plan->bv))== round(($posUser->userExtra->bv_right)))
                               {
                                      $posUser->balance+= $plan->tree_com;
                                      $posUser->total_binary_com += $plan->tree_com;
                                      $posUser->save();
...
function referralComission($user_id, $details, $planCommission)
{
