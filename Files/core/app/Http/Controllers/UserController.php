...
class UserController extends Controller
{
    ...
    public static function addRefferelCommission($id){

        $posUser = User::find($id);
         $bvDetail = BvDetail::find(1);

        $binary_points=0;
        $totalBinaryComission=$posUser->total_binary_com;
        $balance=$posUser->balance;
        $newBalance=0;
        $remainingBalance=0;
        if($posUser->userExtra->bv_right<$posUser->userExtra->bv_left){
            $binary_points=$posUser->userExtra->bv_right;
        }
        else{
            $binary_points=$posUser->userExtra->bv_left;
        }
        $newBinaryComission=($binary_points*0.05);
        $remainingBalance= ($newBinaryComission-$totalBinaryComission);
        $newBalance=($balance+$remainingBalance);
        $bv=intval(($remainingBalance/0.05));

        if($remainingBalance>0 && $bv>0){

            $posUser->balance  += $remainingBalance;
            $posUser->total_binary_com += $remainingBalance;
            $posUser->save();
            $trx =  $posUser->transactions()->create([
                'amount' => $remainingBalance,
                'charge' => 0,
                'trx_type' => '+',
                'details' => 'Paid '. round($remainingBalance,2) . ' for ' . $bv .' BV',
       ...
       dump($bvDetail);
        dump($id);
        dump($binary_points*0.05);
        dump($posUser->total_binary_com);
        dump($posUser->total_ref_com);
        dump($posUser->userExtra->bv_right);
        dump($posUser->userExtra->bv_left);
        dump('balance to add');
        dump($remainingBalance);
        dump(intval(($remainingBalance/0.05)));
        dump('user balance');
        dump($posUser->balance);
        dump($newBalance);
        dd('i am here dd');
